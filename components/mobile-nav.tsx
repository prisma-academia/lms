...
export function MobileNav({
  ...
  return (
    <>
      <div className="sticky top-0 z-40 flex items-center gap-3 border-b-2 border-ink bg-card px-4 py-3 lg:hidden">
        <button
          type="button"
          onClick={() => setOpen((o) => !o)}
          aria-label={open ? "Close menu" : "Open menu"}
          aria-expanded={open}
          className="flex size-11 shrink-0 items-center justify-center rounded-[10px] border-2 border-ink bg-card shadow-brutal-sm [touch-action:manipulation] active:translate-x-px active:translate-y-px active:shadow-none"
        >
