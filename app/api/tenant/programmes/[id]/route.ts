...
export async function PATCH(request: Request, ctx: { params: Promise<{ id: string }> }) {
  try {
    ...
    const before = await prisma.programme.findFirst({ where: { id, tenantId: actor.tenantId } });
    if (!before) throw new DomainError(404, "not_found", "Programme not found.");
    ...
    await audit({
      ...
      after: { status: programme.status, title: programme.title } as object,
      ip: meta.ip,
...
export async function DELETE(request: Request, ctx: { params: Promise<{ id: string }> }) {
  try {
    ...
    await audit({
      ...
      targetType: "Programme",
      targetId: id,
      before: { title: programme.title } as object,
      ip: meta.ip,
      userAgent: meta.userAgent,
    });
